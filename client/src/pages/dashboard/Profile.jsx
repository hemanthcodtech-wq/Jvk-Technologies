    ...
    <div className="min-h-screen bg-bg-cream pt-4 md:pt-16 pb-24 md:pb-12 font-inter">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center mb-6 mt-2 md:hidden">
          ...
          <h1 className="text-xl font-bold text-brand-600">Profile</h1>
        </div>
        ...
        >
          ...
          <div className="w-full md:w-2/5 bg-white/50 p-8 md:p-12 flex flex-col border-b md:border-b-0 md:border-r border-white/60 relative">
            ...
            <div className="flex-1 flex flex-col justify-center">
              <div className="w-16 h-1 bg-brand-600 mb-6 rounded-full hidden md:block"></div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              {menuItems.map((item, index) => (
                ...
                >
                  ...
                  >
                    <div className={`w-12 h-12 rounded-full flex flex-shrink-0 items-center justify-center mr-4 transition-colors duration-300 ${item.title === 'Logout' ? 'bg-red-50 text-red-500 group-hover:bg-red-500 group-hover:text-white' : 'bg-brand-600/10 text-brand-600 group-hover:bg-brand-600 group-hover:text-white'}`}>
                      <item.icon size={24} />
                    </div>
                    ...
                    <FaChevronRight size={14} className={`transition-colors ${item.title === 'Logout' ? 'text-red-200 group-hover:text-red-400' : 'text-gray-300 group-hover:text-brand-600'}`} />
                  </button>
                </motion.div>
              ))}
            </div>
