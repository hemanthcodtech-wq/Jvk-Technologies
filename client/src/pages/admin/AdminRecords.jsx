...
const AdminRecords = () => {
  ...
  const [customForm, setCustomForm] = useState({
    ...
    instructorTitle: 'Lead Technical Trainer',
    instructorSubtitle: 'JVK Technologies Certified',
    sendEmail: true,
  ...
  const generateNewCertId = () => {
    ...
    return `JVK-CERT-${timestamp}${rand}`;
  };
  ...
  const handleSelectCourse = (selectedTitle) => {
    ...
    if (course) {
      const instName = course.instructorId?.name || course.instructor || 'Lead Instructor';
      const instTitle = course.instructorId?.speciality || 'Lead Technical Trainer';
      setCustomForm(prev => ({
        ...prev,
  ...
  const handleOpenCreateModal = () => {
    ...
    const instName = firstCourse?.instructorId?.name || firstCourse?.instructor || 'Lead Instructor';
    const instTitle = firstCourse?.instructorId?.speciality || 'Lead Technical Trainer';
    ...
    setCustomForm({
      ...
      studentId: `JVKST${Date.now().toString().slice(-6)}`,
  ...
  const handleOpenEditModal = (record) => {
    ...
    const instName = fullCourse.instructorId?.name || fullCourse.instructor || record.instructorName || 'Lead Instructor';
    const instTitle = fullCourse.instructorId?.speciality || record.instructorTitle || 'Lead Technical Trainer';
    ...
    setCustomForm({
      ...
      studentId: record.studentEmail ? `JVKST${record.studentEmail.replace(/[^a-zA-Z0-9]/g, '').slice(0, 6).toUpperCase()}` : 'JVKST250501',
      duration: courseDuration,
      instructorName: instName,
      instructorTitle: instTitle,
      instructorSubtitle: 'JVK Technologies Certified',
  ...
  return (
    <div className="space-y-8 pb-24 md:pb-8 font-inter">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl border border-white/80 shadow-[0_4px_20px_rgba(0,0,0,0.02)] flex items-center gap-4">
          <div className="w-13 h-13 rounded-2xl bg-indigo-50 text-indigo-600 border border-indigo-200 flex items-center justify-center text-2xl shrink-0">
            <FaRupeeSign />
          </div>
          <div>
                    ...
                    <td className="p-5">
                      <div className="space-y-1">
                        <span className="font-mono text-xs font-bold text-gray-800 block">
                          {r.invoiceNumber || 'JVK-INV-Generated'}
                        </span>
                        {r.invoiceUrl ? (
                          <a
                            href={r.invoiceUrl}
                ...
                <div className="lg:col-span-7 space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="space-y-2">
                      <input
                        type="text"
                        required
                        placeholder="e.g. Java Full Stack Live Batch - Sept 2026"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input
                        type="text"
                        placeholder="e.g. JVK-CERT-260823"
              ...
              <form onSubmit={handleVerifySearch} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      placeholder="e.g. JVK-CERT-2026-9941"
                      value={verifyInput}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
            ...
            >
              ...
              <div className="relative w-full aspect-[842/595] rounded-3xl overflow-hidden shadow-2xl border-4 border-[#D4AF37] select-none bg-[#FCFAF6]">
                ...
                >
                  {previewRecord.studentEmail ? `JVKST${previewRecord.studentEmail.replace(/[^a-zA-Z0-9]/g, '').slice(0, 6).toUpperCase()}` : 'JVKST250501'}
                </div>
                ...
                >
                  {previewRecord.certificateId || 'JVK-CERT-SAMPLE'}
                </div>
                ...
                >
                  <p className="text-[8px] sm:text-[10.5px] font-bold text-[#0A4F2A] uppercase tracking-wider truncate">
                    {previewRecord.course?.instructorId?.name || previewRecord.course?.instructor || 'Lead Instructor'}
                  </p>
                  <p className="text-[6px] sm:text-[8px] text-gray-600 truncate">
                    {previewRecord.course?.instructorId?.speciality || 'Lead Technical Trainer'}
                  </p>
                </div>
